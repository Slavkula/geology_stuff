...
text = """
Программа убирает красные числа в ведомости.
1. Делай копию ведомости и работай в ней.
2. Выбираешь ведомость по кнопке, выберешь - позеленеет.
3. Начало и конец это строки для проверки от и до.
4. Столбец, в котором делать замены.
5. Запускаешь макрос, откроется твой эксель,
если окошки появляются, то работай с ними как обычно.
6. Программу можно не открывать заново, а просто менять
строки, файл перевыбирать тоже не надо.
7. Делать ничего не надо, программа сохранит и закроет
эксель, когда всё сделает.
"""
